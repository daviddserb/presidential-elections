            ...
            connection.query(`SELECT candidate FROM candidates`, function(err, rows) {
                if (err) {
                    ...
                    if (req.session.loggedInUserInfo.role != "admin") {
                        res.render('userAccount', {registeredUserInfo: req.session.loggedInUserInfo, candidates: rows, popUpMessage: "userAcc"});
                    } else {
                        res.render('adminAccount', {registeredUserInfo: req.session.loggedInUserInfo, candidates: rows, popUpMessage: "adminAcc"});
                    }
                }
            });
        }
